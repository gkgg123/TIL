...
for tc in range(1,T+1):
    ...
    result=DoubleLinkedList()   ### LinkedList의 인스턴스를 생성하고,
    for i in arr:
        result.append(i)   ### 처음 arr를 넣어준다.

    for i in range(M):
        doit=list(input().split())

        if doit[0]=='I':  #### 첫글자가 I이면 insert 해주고
            result.insert(int(doit[1]),int(doit[2]))
        elif doit[0]=='D':  ### D면 지워주고,
            result.remove(int(doit[1]))
        else:             ### C면 바꿔준다.
            result.change(int(doit[1]),int(doit[2]))


    result=result.print_all()  ### 전체를 반환받고,

    if L<len(result):  ## 전체 크기가 L보다 크면 해당 L위치에 존재하는 값을 출력해준다.
